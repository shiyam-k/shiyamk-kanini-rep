  ...
  const Logout = async () => {
    try {
      const response = await axios.get(
        `https://localhost:5000/Authicate/actions/logout?SId=${auth.loginLog.sessionId}`,
        {headers: {
          Authorization: `Bearer ${auth.loginLog.token}`,
        },}
      );
    ...
    <div>
      ...
      >
        <Modal
          ...
          {logoutResponse.status !== null && logoutResponse.status === true ? (
            <Stack sx={style} alignItems="center" justifyContent="space-evenly" className="rounded">
              ...
              <Link to={'/loginComponent/loginPage'} className='btn btn-outline-success' onClick={clearData} >Goto Login ({countdown})</Link>
            </Stack>
          ) : (
            <Stack sx={style} alignItems="center" justifyContent="space-evenly" className="rounded">
              <Typography id="text-response-message" variant="h6" className="text-warning">
